...
function duplicateEncode(word){
  ...
  return result.map(char=>{
    if(result.indexOf(char)===result.lastIndexOf(char)){
      return '(';
    }
    else{
      return ')';
...
function removeSmallest(numbers) {
    ...
    const firstIndexOflowestNum = numbers.indexOf(lowestNum);
    
    const output = [];
  
    for (let index = 0; index < numbers.length; index++) {
        if (index !== firstIndexOflowestNum) {
...
function order(words){
  const numArr=words.split('').filter(char=>Number(char)).sort((a,b)=>a-b);
  let output=[];
...
const solution = function(firstArray, secondArray) {
    ...
    for (let index = 0; index < firstArray.length; index++) {
        result += Math.pow(firstArray[index] - secondArray[index],2) ;
    }
...
function cakes(recipe, available) {
  ...
  const IngredientNames=Object.keys(available);
  let ContainsFoundToBeTrue=recipeNames.every(element=>IngredientNames.includes(element));
  if(ContainsFoundToBeTrue){
      const limit=recipeNames.map(ingredient=>Math.floor(available[ingredient]/recipe[ingredient]));
...
function phone(strng, num) {
  let current, newstrng, Name, start, end, Address;

  // 1. Check if the number exists at all
  if (!strng.includes(num)) {
    return `Error => Not found: ${num}`;
  }

  // 2. Split the phone book into individual lines
  const lines = strng.split("\n");

  // 3. Find the line containing the number
  let line;
  ...
  // 4. Check if the number appears in more than one line
  let count = 0;
  ...
  // 5. Find the actual phone number (+X-num)
  const startindex = line.indexOf(num);
  const plusIndex = line.lastIndexOf("+", startindex);

  current = line
    .split("")
    .splice(plusIndex, num.length + 1)
    .join("");
  ...
  // 7. Remove phone number and name from the line
  newstrng = line.replace(current, "");
  newstrng = newstrng.replace(`<${Name}>`, "");
...
function likes(names) {
  ...
  else if(names.length==1) outputText+=`${names[0]} `;
  else if(names.length==2) outputText+=`${names[0]} and ${names[1]} `;
  else if(names.length==3) outputText+=`${names[0]}, ${names[1]} and ${names[2]} `;
  else{
    const num=names.length-2;
    outputText+=`${names[0]}, ${names[1]} and ${num} `;
  }
...
function sortArray(array) {
  ...
  const oddNumSort=oddNum.sort((a,b)=>a-b);
  let outputArr=[];
  let i=0;
...
function findUniq(arr) {
  // do magic
  const firstArr=arr.filter(num=>arr[0]==num);
  if(firstArr.length===1)return arr[0];
...
function longestConsec(strarr, k) {
  const n = strarr.length; // Define n so the edge-case check works
  if (n === 0 || k > n || k <= 0) return "";
  
  let solArr = [];
  
  // Outer loop stops at n - k + 1 so we always have k elements left to combine
  for (let i = 0; i <= n - k; i++) {
    let combinedStr = ""; // Clear string for the current starting position
    
    // Inner loop runs exactly k times to collect consecutive items
    for (let j = 0; j < k; j++) {
      combinedStr += strarr[i + j]; // Move forward by j from the starting index i
    }
    
    solArr.push(combinedStr);
  }
  
  // Your original reduce logic works perfectly here
  return solArr.reduce((c, n) => n.length > c.length ? n : c, "");
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*Introduction
...
function countSmileys(arr) {
  ...
  for(let face of arr){
    if(face.length===2){
      const [eyes,mouth]=face.split('');
      if(validEyes.includes(eyes) && validMouths.includes(mouth)){
        counter++
      }
    }
    else if(face.length===3){
      const [eyes,nose,mouth]=face.split('');
      if(validEyes.includes(eyes) && validNoses.includes(nose) && validMouths.includes(mouth)){
        counter++
      }
...
var isAnagram = function(test, original) {
  if(test.length==original.length){
    let formatedArr=test.toUpperCase().split('').sort().join('');
    let originalFormated=original.toUpperCase().split('').sort().join('');
    return formatedArr==originalFormated?true:false;
  }
  return false;
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function incrementString(strng) {
    ...
    const incrementedNum = Number(numberStr) + 1;

    const paddedNumber = String(incrementedNum).padStart(numberStr.length, '0');
